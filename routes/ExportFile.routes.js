...
const Employee = require('../models/employee.model'); // Assuming you have an Employee model
...
router.get('/employees/excel', async (req, res) => {
    try {
        const employees = await Employee.find({}, '-_id name code department designation workLocation reportingManager workExperience email');
...
router.get('/employees/csv', async (req, res) => {
    try {
        const employees = await Employee.find({}, '-_id name code department designation workLocation reportingManager workExperience email');
        const csv = json2csv(employees, { fields: ['name', 'department', 'email'] });
            ...
            <html>
                ...
                <body>
                    ...
                    <table border="1">
                        ...
                        ${employees.map(emp => `<tr><td>${emp.name}</td><td>${emp.department}</td><td>${emp.email}</td><td>${emp.code}</td> <td>${emp.designation}</td> <td>${emp.workLocation}</td><td>${emp.reportingManager}</td><td>${emp.workExperience}</td></tr>`).join('')}
                    </table>
                </body>
            </html>
        `;
